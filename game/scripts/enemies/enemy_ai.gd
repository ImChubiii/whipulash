...
## War ein dynamisches load() in _ready(), erneut ausgefuehrt bei JEDEM
## einzelnen Gegner-Spawn - gleiche Idee wie PSX_SHADER oben, nur bisher
## uebersehen.
const ANIM_MANAGER_SCRIPT: GDScript = preload("res://scripts/characters/animation_manager.gd")
...
## _get_separation_velocity() durchsucht dafuer JEDE "enemies"-Gruppe - bei
## N Gegnern im Raum also O(N) pro Aufruf, UND jeder Gegner ruft es selbst
## nochmal auf -> O(N^2) pro Physik-Frame, wenn jeden Frame ungedrosselt neu
## gerechnet wird. Die Kraft ist eine weiche Anti-Ueberlappungs-Andeutung,
## kein hartes Kollisions-System - eine leichte Verzoegerung zwischen echten
## Neuberechnungen (der zuletzt berechnete Wert wird zwischendurch einfach
## weiterverwendet) ist unsichtbar, spart aber die meisten dieser Scans.
## Gleiche Drosselungs-Idee wie unstuck_check_interval oben.
@export var separation_update_interval: float = 0.15
...
# Siehe separation_update_interval oben - haelt den zuletzt berechneten
# Separations-Vektor zwischen echten Neuberechnungen fest.
...
## Praktisch, um RA/RB optisch auseinanderzuhalten. Rueckmeldung "Gegner zu
## hell": Fighter/Stinger/Colossus (dummy.tscn/scout_dummy.tscn/
## tank_dummy.tscn) setzen dafuer jeweils eine eigene, abgedunkelte tint -
## dieser Klassen-Default bleibt bewusst neutral (WEISS = 1,1,1,1), damit
## kuenftige, noch nicht abgedunkelte Gegner nicht ungewollt gefaerbt starten.
...
func _ready() -> void:
	...
	anim_manager = ANIM_MANAGER_SCRIPT.new(self)
...
func _physics_process(delta: float) -> void:
	...
	_separation_timer = max(_separation_timer - delta, 0.0)
	...
	if _state != previous_state:
		_debug("State-Wechsel: %s -> %s (Distanz %.2f, attack_range %.2f, eigene Position %s, Spieler-Position %s)" % [State.keys()[previous_state], State.keys()[_state], distance, attack_range, global_position, (_player.global_position if _player else "?")])
...
## Gedrosselter Zugriff - siehe separation_update_interval. Der Timer wird
## zentral in _physics_process() runtergezaehlt; laeuft er noch, wird einfach
## der zuletzt berechnete Wert zurueckgegeben statt neu ueber alle Gegner zu
## summieren.
func _get_separation_velocity() -> Vector3:
	...
	_separation_cache = _compute_separation_velocity()
	return _separation_cache
