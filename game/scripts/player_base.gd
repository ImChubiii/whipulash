...
# --- Bewegungs-Neigung (Tilt/Lean) --------------------------------------
## Neigt das CharacterModel leicht in die aktuelle Bewegungsrichtung -
## vorwaerts/rueckwaerts als Vor-/Zurueckkippen (Pitch), links/rechts als
## seitliches Kippen (Roll). Rein kosmetisch: wirkt nur auf mesh.rotation.x/z
## (die Y-Achse bleibt komplett der Kamera-/Target-Lock-Logik weiter unten
## ueberlassen), hat also keinerlei Einfluss auf Kollision, Kamera oder
## Angriffsrichtung.
@export_range(0.0, 45.0) var movement_tilt_max_degrees: float = 8.0
## Wie schnell sich die Neigung dem Zielwert annaehert (und beim Loslassen
## der Bewegungstasten wieder auf 0 zurueckfedert) - groesser = schneller/
## direkter, kleiner = traeger/weicher.
@export var movement_tilt_speed: float = 8.0

var _tilt_x: float = 0.0
var _tilt_z: float = 0.0

## Sehr leichter Kamera-Roll in dieselbe Bewegungsrichtung wie das Modell-
## Tilt oben, aber bewusst eigene Variablen (Winkel UND Geschwindigkeit),
## damit sich beides unabhaengig voneinander abstimmen laesst - die Kamera
## soll deutlich dezenter bleiben als das Modell, siehe Rueckmeldung "nur
## sehr subtil". Wird in _physics_process() weich berechnet, aber tatsaechlich
## erst in _process() auf camera.rotation.z angewendet (dort zusammen mit
## Combo-Tilt/Screenshake/Dash-Drill, siehe camera.rotation.z-Kommentar dort).
@export_range(0.0, 15.0) var camera_tilt_max_degrees: float = 1.5
@export var camera_tilt_speed: float = 6.0

var _camera_movement_tilt_degrees: float = 0.0

# --- Target Lock ---
@export var camera_soft_lock_strength: float = 1.5
## Wie schnell sich das Modell zum Target-Lock-Ziel dreht, wenn ein Angriff
## trifft (siehe _physics_process() weiter unten). NICHT das Standard-
## Verhalten (das folgt der Kamera instantan) - nur die Ausnahme beim
## Anvisieren eines getroffenen Gegners. Rueckmeldung "Drehung noch
## schneller" - war 10.0.
@export var model_lock_turn_speed: float = 20.0
...
func _process(delta: float) -> void:
	...
	camera.rotation.z = deg_to_rad(_combo_tilt_degrees + shake_roll_degrees + _dash_roll_degrees + _camera_movement_tilt_degrees)
...
func _physics_process(delta: float) -> void:
	...
	# Rueckmeldung "Kamera soll sich beim Attackieren garnicht mehr bewegen,
	# ausser bei Projektil-Charakteren (Giselle/Winter)": camera_soft_lock_
	# strength ist dafuer schon der richtige Hebel - auf 0.0 gesetzt in
	# char_karina.tscn/char_ningning.tscn haelt lerp_angle() unten die Kamera
	# unveraendert, ohne die Modell-Drehung zum Ziel (siehe weiter unten,
	# _current_target-Zweig) anzutasten. Giselle/Winter behalten den
	# Default (1.5) unveraendert in ihren Szenen.
	if _current_target and is_instance_valid(_current_target):
	...
	# Modell-Rework: Standardverhalten bleibt "schaut in Kamera-/
	# Schussrichtung" (siehe BUGFIX-Kommentar im else-Zweig). AUSNAHME: landet
	# ein Angriff/Schuss auf einem Gegner, setzt combat_base.gd::
	# _on_hit_landed() bzw. combat_giselle.gd/combat_winter.gd::
	# _lock_model_to() bereits _current_target ueber set_target() - das ist
	# dieselbe, schon lange bestehende Target-Lock-Infrastruktur, die auch den
	# Soft-Lock der Kamera oben treibt. Ist ein Ziel gesetzt, dreht sich das
	# Modell WEICH zu ihm (statt starr der Kamera zu folgen), bis es stirbt
	# oder ein anderer Gegner getroffen wird (siehe clear_target()-Aufrufer).
	if _current_target and is_instance_valid(_current_target):
		...
		# BUGFIX (war 180 Grad verdreht): camera_pivot.rotation.y selbst
		# entspricht der +Z-Achse von camera_pivot - das ist aber bei diesem
		# Rig die Richtung VON der Kamera WEG, nicht die Blick-/Zielrichtung
		# (siehe den Soft-Lock weiter oben: desired_yaw wird bewusst aus
		# -to_target_cam berechnet, camera_pivot zeigt beim Locken also mit +Z
		# WEG vom Ziel). Die eigentliche Kamera-/Schussrichtung ist
		# -camera.global_transform.basis.z (siehe combat_giselle.gd/
		# combat_winter.gd, die genau damit zielen) - das entspricht
		# camera_pivot.rotation.y + PI. Ohne das +PI schaute das Modell beim
		# Schiessen/Angreifen also von Gegnern WEG statt sie anzusehen.
		mesh.rotation.y = camera_pivot.rotation.y + PI

	# --- Bewegungs-Neigung (Tilt/Lean) ---
	# input_dir.y ist NEGATIV beim Vorwaertslaufen (W = "ui_up", siehe
	# Input.get_vector()-Aufruf oben) - deshalb mit umgekehrtem Vorzeichen in
	# den Pitch-Zielwinkel uebernommen, damit "W" nach vorne neigt. Beim Roll
	# (links/rechts) dagegen OHNE Vorzeichenumkehr: input_dir.x ist bei "A"
	# ("ui_left") NEGATIV, und mesh.rotation.z soll bei "A" ebenfalls negativ
	# werden, damit das Modell tatsaechlich nach links kippt (Rueckmeldung:
	# mit umgekehrtem Vorzeichen war links/rechts vertauscht). Baut auf der
	# Annahme auf, dass mesh.rotation.y (oben gesetzt) ungefaehr der
	# Kamerarichtung entspricht - waehrend eines Target-Locks (Modell schaut
	# aufs Ziel statt zur Kamera) kann die gefuehlte Vor-/Seitenrichtung
	# dadurch leicht von der echten Bewegungsrichtung abweichen; bei der
	# kurzen Lock-Dauer nicht spuerbar.
	# Skaliert mit effective_speed/speed statt dem rohen Tastendruck, damit
	# das Modell nicht in die gehaltene Richtung neigt, waehrend der
	# Charakter durch Stun/Slow tatsaechlich gar nicht (oder nur gebremst)
	# vorankommt - sonst wirkt es, als "wolle" er sich bewegen, obwohl er
	# sichtbar auf der Stelle steht.
	var move_factor: float = clampf(effective_speed / speed, 0.0, 1.0) if speed > 0.0 else 0.0
	var target_tilt_x: float = -input_dir.y * deg_to_rad(movement_tilt_max_degrees) * move_factor
	var target_tilt_z: float = input_dir.x * deg_to_rad(movement_tilt_max_degrees) * move_factor
	var tilt_weight: float = clampf(movement_tilt_speed * delta, 0.0, 1.0)
	_tilt_x = lerp(_tilt_x, target_tilt_x, tilt_weight)
	_tilt_z = lerp(_tilt_z, target_tilt_z, tilt_weight)
	mesh.rotation.x = _tilt_x
	mesh.rotation.z = _tilt_z

	# --- Kamera-Neigung (leichter Roll in Bewegungsrichtung) ---
	# Bewusst dasselbe Vorzeichen-Vorbild wie play_dash_drill_effect()'s
	# Kamera-Roll (-signf(strafe) * dash_drill_degrees, siehe dort): bei
	# "D"/ui_right (input_dir.x = +1) ergibt sich ein NEGATIVER Zielwinkel,
	# bei "A"/ui_left (input_dir.x = -1) ein POSITIVER - gleiches Rollgefuehl
	# wie beim seitlichen Dash, nur deutlich subtiler. Nur der eigene, neue
	# Summand _camera_movement_tilt_degrees wird hier gesetzt - camera.
	# rotation.z selbst wird weiterhin ausschliesslich in _process() aus ALLEN
	# Roll-Quellen (Combo-Tilt, Screenshake, Dash-Drill, hier) neu
	# zusammengesetzt, siehe dortiger Kopfkommentar zum "kein Tween auf
	# dieselbe Eigenschaft"-Grund.
	var target_camera_tilt: float = -input_dir.x * camera_tilt_max_degrees * move_factor
	_camera_movement_tilt_degrees = lerp(_camera_movement_tilt_degrees, target_camera_tilt, clampf(camera_tilt_speed * delta, 0.0, 1.0))
