...
## Debug-Direktstart: ueberspringt Hauptmenue UND Party-Auswahl komplett,
## startet sofort einen Normal-Run und teleportiert automatisch in den
## Gegner-Sandbox-Raum (siehe enemy_sandbox_room.gd), sobald Level und
## Spieler bereitstehen - zum schnellen Gegner-/Balancing-Testen. Wieder auf
## false stellen, um normal ueber das Hauptmenue zu spielen/zu exportieren.
@export var debug_boot_into_sandbox: bool = false
...
func _ready() -> void:
	...
	if debug_boot_into_sandbox:
		# EnemySandboxRoom.request_auto_teleport() MUSS VOR _start_run()
		# laufen: die wartet auf das naechste generierte Level, und
		# _start_run() wechselt per change_scene_to_file() sofort dorthin -
		# dieser Node (samt _ready()-Kaskade) ist danach abgebaut.
		EnemySandboxRoom.request_auto_teleport()
		_start_run(false)
		return
	...
	# Wenn der aktive Charakter sich ändert (Wechsel, Unlock, Last-Stand),
	# soll der Character-Screen live folgen — sofern er gerade sichtbar ist.
...
## Wird von PartyManager.active_character_changed gefeuert (Index-basiert).
## Aktualisiert den Character-Screen wenn er gerade sichtbar ist.
func _on_active_character_changed(index: int) -> void:
	if _current_screen_id != "character":
		return
...
## Wird von PartyManager.active_player_changed gefeuert (Node-basiert).
## Selbe Logik wie oben, aber anhand des aktuellen PartyManager-Index.
func _on_active_player_changed(_player: CharacterBody3D) -> void:
	_on_active_character_changed(PartyManager.get_active_index())
...
# ============================================================================
# Play-Submenu — Normal / Speedrun / Tutorial
# ============================================================================
## Normal und Speedrun sind gesperrt bis GameStats.tutorial_completed == true.
## Solange gesperrt, erscheinen sie durchgestrichen und sind nicht klickbar.
## Tutorial ist immer aktiv.
func _build_play_screen() -> void:
	var box: VBoxContainer = _create_screen("play")
	box.add_child(_make_title("Play", 22))

	# --- Normal ----------------------------------------------------------------
	var normal_label_text: String = "Normal"
	var normal_button: Button = _make_button(box, normal_label_text)
	normal_button.pressed.connect(func() -> void: _start_run(false))
	_apply_play_mode_state(normal_button, GameStats.tutorial_completed)

	# --- Speedrun --------------------------------------------------------------
	var speedrun_button: Button = _make_button(box, "Speedrun")
	speedrun_button.pressed.connect(func() -> void: _start_run(true))
	_apply_play_mode_state(speedrun_button, GameStats.tutorial_completed)
	...
	# --- Tutorial --------------------------------------------------------------
	var tutorial_button: Button = _make_button(box, "Tutorial")
	tutorial_button.pressed.connect(_start_tutorial)

	var tutorial_hint := Label.new()
	tutorial_hint.text = "Tutorial: lernt alle vier Charaktere in fester Reihenfolge kennen."
	tutorial_hint.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	tutorial_hint.autowrap_mode = TextServer.AUTOWRAP_WORD
	tutorial_hint.add_theme_font_size_override("font_size", 13)
...
## Setzt den visuellen Zustand eines Normal-/Speedrun-Buttons je nachdem,
## ob das Tutorial bereits abgeschlossen wurde.
##   locked = true  → durchgestrichen, deaktiviert, gedimmt
##   locked = false → normal spielbar
func _apply_play_mode_state(button: Button, unlocked: bool) -> void:
	var label: Label = _button_labels.get(button)
	if label == null or not is_instance_valid(label):
		return
	if unlocked:
		_set_button_disabled(button, false)
	else:
		_set_button_disabled(button, true)
		# Durchstreichung: RichTextLabel ueber demselben Slot legen
		if label.get_parent() != null:
			...
			rtl.text = "[s]%s[/s]" % label.text
			rtl.add_theme_font_size_override("normal_font_size", 20)
			rtl.add_theme_color_override("default_color", Color(MENU_TEXT_COLOR, 0.35))
			rtl.position = label.position
			rtl.size = label.size
			label.visible = false
			label.get_parent().add_child(rtl)


## Setzt TutorialFlow.pending und wechselt in dieselbe Gameplay-Szene wie ein
## normaler Run - level_generator.gd::generate_tutorial_stage() baut dort die
## feste Tutorial-Raumfolge und die anfangs nur Ningning umfassende Party.
## Kein RunRestart-Sonderfall wie in _start_run() noetig:
## main_menu.gd wird nachweislich nirgends als lebendiges Pause-Overlay
## instanziert (immer per change_scene_to_file erreicht, siehe Klassen-
## Kopfkommentar oben) - ein eventuell noch laufender Run ist beim Aufruf
## hier also bereits durch den Szenenwechsel hierher beendet.
## Setzt nur ein Flag und laedt DIESELBE Gameplay-Szene wie ein normaler Run
## - LevelGenerator._ready() liest TutorialFlow.pending beim Autostart aus
## und generiert damit statt Etage 1 die feste Tutorial-Raumfolge (Etage 0,
## siehe level_generator.gd::generate_tutorial_stage()).
func _start_tutorial() -> void:
	TutorialFlow.pending = true
...
func _refresh_character_screen() -> void:
	...
	# Nur die Standard-Beschreibung des Charakters zeigen, keine Tutorial-Texte.
