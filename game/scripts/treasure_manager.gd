...
## Tutorial-Modus: eigenes Signal-Paar statt die beiden ItemData-typisierten
## oben zu ueberladen - room.character_unlock spawnt einen CharacterPedestal
## statt eines Item-Sockels, siehe _spawn_pedestal() unten.
signal character_pedestal_spawned(data: CharacterData, room: Node)
signal character_taken(data: CharacterData)
...
func _spawn_pedestal(room: RoomInstance) -> void:
	...
	# Tutorial-Modus: character_unlock (RoomInstance @export, siehe
	# room_instance.gd) macht diesen Raum zu einem Charakter-Unlock statt
	# eines normalen Item-Tresorraums - komplett vor der Item-Auswahl
	# abgezweigt, _pick_item()/_weighted_pick()/_reserved_ids bleiben fuer
	# diesen Raum unberuehrt. Gleiches Muster wie is_sacrifice_room oben.
	if room.character_unlock != null:
		var cpedestal: CharacterPedestal = CharacterPedestal.create(room.character_unlock)
		room.add_child(cpedestal)
		cpedestal.global_position = _find_spawn_position(room)
		cpedestal.character_taken.connect(_on_character_taken)
		character_pedestal_spawned.emit(room.character_unlock, room)
		_debug("  -> CharacterPedestal gesetzt: '%s' bei %s." % [
			room.character_unlock.character_name, cpedestal.global_position
		])
		return
...
func _on_character_taken(data: CharacterData, _pedestal: CharacterPedestal) -> void:
	character_taken.emit(data)
	_debug("'%s' vom Charakter-Sockel freigeschaltet." % data.character_name)
