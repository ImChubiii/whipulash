...
## id -> Anzahl im Inventar. Haelt has_item()/count_item() O(1) statt einer
## linearen Suche ueber das gesamte Inventar - beide werden von
## item_behaviours.gd aus _physics_process() und mehreren Hit-/Damage-Signal-
## Handlern gerufen, potenziell mehrfach pro Sekunde gegen ein spaetes
## Run-Inventar mit 20-40+ Eintraegen. Muss bei JEDER Aenderung an inventory
## (add_item(), _remove_from_inventory(), reset_run(), clear_inventory())
## mitgepflegt werden, sonst laeuft er auseinander.
...
func _connect_party_manager() -> void:
	...
	# is_instance_valid() zuerst - siehe item_behaviours.gd::_player() fuer
	# die Begruendung (freed Object).
...
func bind_player(new_player: CharacterBody3D) -> void:
	...
	_enforce_character_exclusive_items()

	player_ready.emit(player)


## Karinas "Reflexe" (ID_KARINA_LIFESTEAL) ist eine charakter-gebundene,
## unsichtbare Passive (siehe char_karina.gd::_grant_passive_lifesteal()) -
## sie darf NUR existieren, waehrend Karina aktiv ist. Das Inventar gehoert
## aber dem RUN, nicht der Figur (siehe Kopfkommentar oben) - ohne diese
## Pruefung wuerde die Passive nach dem ersten Karina-Spawn fuer den Rest
## des Runs auch bei den anderen drei Charakteren im Inventar haengen
## bleiben. Bei jedem Charakterwechsel geprueft statt nur beim Verlassen
## von Karina, damit auch ein uebersprungener/fehlgeschlagener Grant nie zu
## falsch-negativen Zustaenden fuehrt.
func _enforce_character_exclusive_items() -> void:
	var party: Node = get_node_or_null("/root/PartyManager")
	var data: CharacterData = party.get_active_data() if party else null
	if data != null and data.character_id == &"karina":
		return
	for item: ItemData in inventory:
		if item.id == ItemCatalog.ID_KARINA_LIFESTEAL:
			print("[Items] Charakterwechsel weg von Karina (aktiv: %s) - entferne exklusive Passive '%s' aus dem Inventar." % [data.character_id if data else "?", item.id])
...
func has_item(item_id: String) -> bool:
	return int(_inventory_counts.get(item_id, 0)) > 0
...
func add_item(item: ItemData, silent: bool = false) -> bool:
	...
	_inventory_counts[item.id] = int(_inventory_counts.get(item.id, 0)) + 1
...
func _remove_from_inventory(item: ItemData) -> void:
	...
	var remaining: int = int(_inventory_counts.get(item.id, 0)) - 1
	if remaining > 0:
		_inventory_counts[item.id] = remaining
	else:
		_inventory_counts.erase(item.id)
