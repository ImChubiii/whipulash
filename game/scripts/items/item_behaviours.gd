...
func _player() -> CharacterBody3D:
	if _items == null:
		return null
	var p = _items.player
	# is_instance_valid() MUSS zuerst geprueft werden: 'is' auf einer bereits
	# freigegebenen Instanz ist in GDScript selbst ein Laufzeitfehler ("Left
	# operand of 'is' is a previously freed instance"), nicht einfach false -
	# die alte Reihenfolge stuerzte ab, sobald _items.player noch auf den
	# gerade beim Charakterwechsel freigegebenen Spieler zeigte.
