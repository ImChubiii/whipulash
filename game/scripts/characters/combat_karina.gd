...
## Ausfuehrliche Konsolenausgaben zu Stance-/Stealth-Statuswechseln,
## getroffenen/markierten Gegnern (inkl. Position) und Damage-Kalkulationen -
## wie ueberall sonst im Projekt per Instanz zuschaltbar statt global
## (siehe enemy_ai.gd/health.gd fuer dasselbe Muster).
...
func _enter_stance() -> void:
	...
	_debug("Acid Rush Mode BETRETEN - Position %s, Aura-Radius %.1f, Tick-Schaden %.1f alle %.2fs, Dauer %.1fs" % [player.global_position if player else "?", acid_aura_radius, acid_damage_per_tick, acid_tick_interval, stance_max_duration])
...
func _exit_stance() -> void:
	...
	_free_aura_visual()
	_debug("Acid Rush Mode VERLASSEN - Wiedereintritts-Sperre %.1fs" % stance_reentry_cooldown)
...
func _tick_acid_aura(delta: float) -> void:
	...
	for enemy: Node3D in EnemyQuery.enemies_within_flat(player.global_position, acid_aura_radius, 3.0, 5.0):
		if enemy.has_method("apply_status_effect"):
			...
			_debug("Acid-Tick trifft '%s' bei Position %s (Distanz %.2f) - %.1f Schaden/Tick fuer %.1fs" % [enemy.name, enemy.global_position, player.global_position.distance_to(enemy.global_position) if player else -1.0, acid_damage_per_tick, acid_effect_duration])
...
func _start_stealth() -> void:
	...
	_debug("Phantom Execute BETRETEN - Position %s, Beruehrungsradius %.1f, max. Dauer %.1fs" % [player.global_position if player else "?", stealth_touch_radius, stealth_max_duration])
...
func _tick_stealth(delta: float, force_end: bool) -> void:
	...
	for enemy: Node3D in EnemyQuery.enemies_within_flat(player.global_position, stealth_touch_radius, 3.0, 5.0):
		...
		if not _marked_enemy_ids.has(id):
			...
			_debug("Gegner '%s' markiert bei Position %s (Distanz %.2f) - insgesamt %d markiert" % [enemy.name, enemy.global_position, player.global_position.distance_to(enemy.global_position) if player else -1.0, _marked_enemy_ids.size()])
...
func _end_stealth() -> void:
	...
	_debug("Phantom Execute VERLASSEN - Position %s, markierte Gegner: %d, Cooldown danach %.1fs" % [player.global_position if player else "?", _marked_enemy_ids.size(), stealth_reentry_cooldown])
	_spawn_stealth_toggle_vfx()
...
func _spawn_decloak_explosion() -> void:
	...
	bomb.damage = DECLOAK_EXPLOSION_DAMAGE * _damage_multiplier()
	bomb.damages_player = false
	bomb.thrower = player
	get_tree().current_scene.add_child(bomb)
	bomb.global_position = player.global_position
	_debug("Decloak-Explosion bei Position %s - Radius %.2f, Basis-Schaden %.1f x Multiplikator %.2f = %.1f" % [bomb.global_position, bomb.explosion_radius, DECLOAK_EXPLOSION_DAMAGE, _damage_multiplier(), bomb.damage])
	bomb.trigger_now()
...
func _detonate() -> void:
	...
	_debug("Detonation ausgeloest - %d markierte Gegner, Basis-Schaden %.1f x Multiplikator %.2f = %.1f pro Gegner" % [_marked_enemy_ids.size(), detonation_damage, _damage_multiplier(), dmg])
	...
	for id: int in _marked_enemy_ids:
		...
		_debug("Detonation trifft '%s' bei Position %s - %.1f Schaden" % [enemy_3d.name, enemy_3d.global_position, dmg])
		if _item_behaviours:
			_item_behaviours.try_karina_lifesteal()
...
# Gemeinsame Helfer
# ============================================================================
# _damage_multiplier() lebt jetzt in combat_base.gd (identisch dupliziert in
# Giselle/Karina/Winter - siehe dortiger Kommentar).


func _debug(msg: String) -> void:
	if debug_logging:
		print("CombatKarina DEBUG [%s]: %s" % [player.name if player else "?", msg])
