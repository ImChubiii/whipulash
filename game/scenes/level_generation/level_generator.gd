...
## Bitmask-Werte pro Richtung - referenziert RoomInstance.EXIT_NORTH/SOUTH/
## EAST/WEST als kanonische Quelle, statt dieselben Literale (1/2/4/8) ein
## zweites Mal von Hand zu duplizieren. Vorher standen hier eigene Literale
## mit einem Kommentar "Muss 1:1 zu RoomInstance._FLAG_BY_KEY passen" - genau
## das Muster aus zwei unabhaengig zu pflegenden Werten, das an anderer
## Stelle im Projekt (siehe cell_size/elevation_step oben) bereits als
## Bug-Ursache erkannt und vermieden wurde.
const DIR_FLAGS := {
	"north": RoomInstance.EXIT_NORTH,
	"south": RoomInstance.EXIT_SOUTH,
	"east": RoomInstance.EXIT_EAST,
	"west": RoomInstance.EXIT_WEST,
}
...
##
## War default true - dadurch feuerte das komplette Tuer-Protokoll bei
## JEDER Etagen-Generierung UND jedem Raum-Clear im normalen Spielbetrieb.
## Wie ueberall sonst im Projekt (siehe enemy_ai.gd/health.gd/
## combat_karina.gd) jetzt per Default aus, bei Bedarf gezielt zuschaltbar.
@export var debug_doors: bool = false
## Nach jedem Raum-Clear erneut protokollieren.
@export var debug_doors_on_clear: bool = false

## Allgemeine Konsolenausgaben (Seed, Layout-Groesse, Raumzahl, NavMesh-Bake,
## Etagen-/Sieg-Meilensteine) - wie ueberall sonst im Projekt per Instanz
## zuschaltbar statt global.
...
## --- Tutorial-Modus -------------------------------------------------
## Von generate_tutorial_stage() befuellt, von _instantiate_layout() als
## Ueberschreibung der normalen Zufallspfade gelesen (siehe dort). Fuer jede
## normale Etage leer und damit wirkungslos.
var tutorial_room_types: Array[int] = []
var tutorial_fixed_enemies: Dictionary = {}      # Vector2i -> Array[Callable]
var tutorial_character_unlocks: Dictionary = {}  # Vector2i -> CharacterData
var _tutorial_ui: TutorialCharacterIntro = null
...
func _ready() -> void:
	...
	randomize()
	_debug("[LevelGenerator] Run-Seed: %d (Code: %s)" % [_run_seed, DetRng.seed_to_code(_run_seed)])
...
func generate_tutorial_stage() -> void:
	...
	# gleich intern nochmal geht (siehe dort) - so kennt dieser Aufrufer die
	# Position jeder Kettenzelle, OHNE die Lauf-Logik des Generators
	# duplizieren zu muessen (nur der simple Richtungs-Offset-Schritt wird
	# hier gespiegelt).
	var positions: Array[Vector2i] = []
	var cursor := Vector2i.ZERO
	for i in range(tutorial_room_types.size()):
		var dir: String = directions[i] if i < directions.size() else "east"
		cursor += DIR_OFFSETS[dir]
		positions.append(cursor)

	tutorial_fixed_enemies.clear()
	tutorial_character_unlocks.clear()
	for i in range(tutorial_room_types.size()):
		var builders: Array[Callable] = TutorialContent.fixed_enemies_for(i)
		if not builders.is_empty():
			tutorial_fixed_enemies[positions[i]] = builders
		var unlock: CharacterData = TutorialContent.character_unlock_for(i)
		if unlock != null:
			tutorial_character_unlocks[positions[i]] = unlock

	_current_layout = grid_generator.generate_fixed_layout(tutorial_room_types, directions)
	_debug("[LevelGenerator] Tutorial-Layout generiert: %d Zellen." % _current_layout.size())
	_instantiate_layout(_current_layout)

	_unlock_tutorial_treasure_exits(tutorial_room_types, directions, positions)


## Tresorraeume liegen im Tutorial INLINE (Eingang + Ausgang) statt als
## Sackgasse - das bricht _apply_door_kinds()'s Annahme, dass JEDE Tuer an
## einem TREASURE-Raum entweder von INNEN (immer offen) oder von einem noch
## nicht besuchten Kampfraum aus (hack-gated auf dessen EIGENEN Clear-
## Zustand) betreten wird. Fuer die AUSGANGS-Tuer (naechster Raum -> Tresor)
## ist genau diese zweite Annahme falsch: der naechste Raum kann nicht
## "gecleart" sein, bevor man ihn durch exakt diese Tuer betreten hat - eine
## Tuer, die sich nie oeffnen laesst (siehe Kommentar ueber
## RoomGridGenerator._place_special_rooms()). Deshalb wird hier NACH
## _apply_door_kinds() (laeuft innerhalb von _instantiate_layout()) die
## Rueckwaerts-Tuer jedes Nachfolgeraums explizit freigestellt - bewusst nur
## hier im Tutorial-Pfad, _apply_door_kinds()/_on_room_cleared() bleiben fuer
## jede normale, zufaellig generierte Etage unangetastet (dort kommt ein
## 2-Tuer-Tresorraum nie vor).
##
## Betrifft den Uebergang Tresor(Item, Index 7) -> Bossraum(Index 8) nur als
## harmloser No-Op: Bossraeume sind selbst is_inside_special, ihre eigene
## Tuer zurueck zum Tresorraum ist also schon automatisch hack-frei.
func _unlock_tutorial_treasure_exits(room_types: Array[int], directions: Array[String], positions: Array[Vector2i]) -> void:
	for i in range(room_types.size()):
		if room_types[i] != RoomData.RoomType.TREASURE:
			continue
		var next_index: int = i + 1
		if next_index >= room_types.size():
			continue

		var next_room: RoomInstance = _instances.get(positions[next_index])
		if next_room == null or not is_instance_valid(next_room):
			continue

		# directions[next_index] zeigt von Zelle i zu Zelle next_index - die
		# Rueckwaerts-Tuer im naechsten Raum liegt also in der entgegen-
		# gesetzten Richtung.
		var back_dir: String = OPPOSITE_DIR[directions[next_index]]
		next_room.set_door_kind(back_dir, Door.DoorKind.NORMAL)
...
func _setup_tutorial_ui() -> void:
	...
	if PartyManager.active_character_changed.is_connected(_on_tutorial_active_character_changed) == false:
		PartyManager.active_character_changed.connect(_on_tutorial_active_character_changed)

func _on_tutorial_active_character_changed(index: int) -> void:
	if _tutorial_ui == null or not is_instance_valid(_tutorial_ui):
		return
	var data: CharacterData = PartyManager.get_data(index)
	if data != null:
...
func _on_tutorial_character_unlocked(data: CharacterData) -> void:
	if _tutorial_ui == null or not is_instance_valid(_tutorial_ui):
		return
	_tutorial_ui.show_character(data.character_name, TutorialContent.description_for(data))
	# next_character_index fuer den naechsten Tutorial-Start (falls abgebrochen)
	# oder falls das Tutorial erneut gespielt wird.
	var tf: Node = get_node_or_null("/root/TutorialFlow")
	if tf != null:
		match String(data.character_id):
			"giselle":
				tf.set("next_character_index", 1)
			"karina":
				tf.set("next_character_index", 2)
			"winter":
				tf.set("next_character_index", 3)
...
func _instantiate_layout(layout: Dictionary) -> void:
	...
	for grid_pos in layout.keys():
		...
		# Tutorial-Modus: macht diese Zelle zu einem Charakter-Unlock-
		# Tresorraum statt eines normalen Item-Tresors - treasure_manager.gd
		# liest das Feld, sobald sein node_added-Handler diesen Raum sieht.
		# Wirkungslos (bleibt null) fuer jede normale, nicht-Tutorial-Etage.
		if tutorial_character_unlocks.has(grid_pos):
			room.character_unlock = tutorial_character_unlocks[grid_pos]
		...
		# Tutorial-Modus: EXAKTE Gegnerliste statt Threat-Budget-Wuerfeln.
		# Wirkungslos (Dictionary bleibt leer) fuer jede normale Etage.
		if tutorial_fixed_enemies.has(grid_pos):
			room.prepare_fixed_enemies(tutorial_fixed_enemies[grid_pos], cell.room_type == RoomData.RoomType.BOSS)
		else:
	...
	_debug("[LevelGenerator] %d/%d Raeume instanziert. Gegner-Skalierung: HP x%.2f, Schaden x%.2f" % [
...
func _on_room_cleared(room: RoomInstance) -> void:
	if _map_cells.has(room.grid_position):
		...
		if _map_cells[room.grid_position]["type"] == RoomData.RoomType.BOSS:
			...
			_debug("[LevelGenerator] Stage %d gecleared (Bossraum bei %s)." % [current_stage, room.grid_position])
...
func _spawn_victory_trophy(room: RoomInstance) -> void:
	...
	_debug("[LevelGenerator] Sieg-Trophaee gespawnt bei %s (Raum %s, Bodenhoehe %.1f)." % [center, room.grid_position, room.global_position.y])
...
func _rebake_navigation() -> void:
	...
	_debug("[LevelGenerator] NavMesh gebakt (%d Quell-Nodes in '%s')." % [get_tree().get_nodes_in_group(NAV_SOURCE_GROUP).size(), NAV_SOURCE_GROUP])
