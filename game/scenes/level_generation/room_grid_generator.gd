...
## TUTORIAL-MODUS: erzeugt eine VOLLSTAENDIG KONSEKUTIVE Kette ohne jede
## Zufallsauswahl und ohne Verzweigung - jede Zelle ist nur ueber die
## vorherige/naechste erreichbar. Fuer scripts/tutorial_content.gd, das dem
## Generator eine feste Raumfolge vorgeben will statt eines zufaellig
## gewachsenen Baums.
##
## room_types[i] bestimmt den Raumtyp jeder Zelle (0-basiert ab der ERSTEN
## Zelle nach dem Start). directions[i] (optional, faellt auf EAST zurueck)
## bestimmt die Richtung von Zelle i-1 zur Zelle i - Index 0 = Richtung vom
## Startraum zur ersten Zelle. Ein monoton wachsendes Richtungsmuster (z.B.
## nur EAST/NORTH) garantiert automatisch Ueberschneidungsfreiheit, ohne dass
## diese Funktion selbst gegen Kollisionen pruefen muss.
##
## ANDERS als generate_layout(): TREASURE-Zellen duerfen hier (im Unterschied
## zur zufaelligen Etage, siehe _place_special_rooms()-Kommentar weiter
## unten) MEHR als eine Tuer haben, weil sie inline in der Kette liegen
## sollen. Der LevelGenerator gleicht die daraus resultierende Hack-Tuer-
## Falle (siehe dortiger Kommentar zu generate_tutorial_stage()) in einem
## eigenen Nachbereitungsschritt aus - diese Funktion selbst muss davon
## nichts wissen.
func generate_fixed_layout(room_types: Array[int], directions: Array[String] = []) -> Dictionary:
	_occupancy.clear()
	var cells: Dictionary = {}
	...
	for i in range(room_types.size()):
		...
		var next_pos: Vector2i = current_pos + DIRECTION_OFFSETS[dir]
		var cell := RoomCell.new()
		cell.grid_pos = next_pos
		cell.room_type = room_types[i]
		cells[next_pos] = cell
		_link_neighbors(cells, current_pos, next_pos, dir)
		current_pos = next_pos

	# Gleiche Buchfuehrung wie der Sicherheits-Vorblock von _assign_footprints()
	# (siehe dort) - jede Zelle bleibt einzellig, damit Minimap/get_occupant()
	# unveraendert funktionieren.
	for pos in cells.keys():
		var c: RoomCell = cells[pos]
		c.footprint = Vector2i.ONE
		c.covered_cells = [pos]
		_occupancy[pos] = pos
