...
## Tutorial-Modus: ist hier ein CharacterData gesetzt, spawnt
## treasure_manager.gd einen CharacterPedestal (Charakter-Unlock) statt
## eines normalen Item-Pedestals - gleiches Muster wie is_sacrifice_room
## oben. Default null, harmlos fuer alle anderen Raeume.
@export var character_unlock: CharacterData = null
...
@export var shop_room_icon_color: Color = Color(0.35, 0.85, 0.55)
...
## Tutorial-Modus: siehe prepare_fixed_enemies(). Nicht-leer => Threat-Budget
## wird uebersprungen, es spawnt exakt diese Liste.
var _pending_fixed_builders: Array[Callable] = []
...
func _update_entry_presence(inside: bool) -> void:
	...
	if inside:
		...
		# Tresorraeume haben nie einen Kampf-Gate (_requires_clear bleibt
		# false, siehe _table_for_type()/_budget_for_type()) und feuern
		# deshalb nie room_cleared, ueber das _on_self_room_cleared() die
		# Dimmplatte sonst permanent entfernt. "Grau bis betreten" statt
		# "grau bis gecleart" ist bei ihnen ohnehin dasselbe - nichts zu
		# bekaempfen -, also hier direkt beim ersten Betreten entfernen.
		if room_type == RoomData.RoomType.TREASURE:
			_on_self_room_cleared(self)
...
## Tutorial-Modus: ersetzt das Threat-Budget-Wuerfeln (prepare_enemies()
## oben) durch eine EXAKTE Liste. Jeder Callable liefert einen fertigen,
## noch nicht in den Baum gehaengten Gegner-Node (z.B. `func() -> Node3D:
## return DUMMY_SCENE.instantiate()` oder `func() -> Node3D: return
## MortarBot.new()` fuer die sechs Prototypen ohne eigene .tscn) - siehe
## scripts/tutorial_content.gd.
func prepare_fixed_enemies(builders: Array[Callable], is_boss_room: bool = false) -> void:
	_pending_is_boss_room = is_boss_room
	if not _spawn_seed_set:
		_spawn_rng.randomize()
		_spawn_seed_set = true

	if builders.is_empty() or enemy_spawn_points.is_empty():
		_is_cleared = true
		_lock_exits(false)
		return

	_requires_clear = true
	_pending_fixed_builders = builders
	_lock_exits(false)
	_build_room_state_overlay()
...
func _spawn_prepared_enemies() -> void:
	if not _pending_fixed_builders.is_empty():
		_spawn_fixed_enemies()
		return
...
## Tutorial-Modus-Gegenstueck zu _spawn_prepared_enemies() oben: keine
## Budget-Wuerfelei, jeder Builder aus _pending_fixed_builders liefert genau
## einen Gegner-Node, der ueber denselben Registrierungspfad wie ein
## normaler Threat-Budget-Spawn laeuft (_spawn_enemy_node()).
func _spawn_fixed_enemies() -> void:
	var free_points: Array[Marker3D] = enemy_spawn_points.duplicate()
	DetRng.shuffle(free_points, _spawn_rng)
	var taken_positions: Array[Vector3] = []

	for builder in _pending_fixed_builders:
		var point: Marker3D = _take_spawn_point(free_points, taken_positions, 0.0)
		if point == null:
			break
		taken_positions.append(point.global_position)
		var enemy: Node3D = builder.call()
		if enemy == null:
			continue
		_spawn_enemy_node(enemy, point)
...
## Gemeinsamer Registrierungspfad fuer JEDEN Gegner, egal ob er ueber das
## Threat-Budget (_spawn_one() oben) oder ueber eine Tutorial-Fixliste
## (_spawn_fixed_enemies()) zustande kam - Parent-Anhaengen, boss-Gruppe,
## Positionierung, Stage-Skalierung und alle drei Clear-Signale.
func _spawn_enemy_node(enemy: Node3D, point: Marker3D) -> void:
...
func set_room_type(type: int) -> void:
	...
	# BUGFIX "Tresorraum zeigt sich auf der Minimap sofort voll aufgedeckt,
	# bevor der Spieler ihn betreten hat": _build_room_state_overlay() lief
	# bisher NUR aus prepare_enemies()/prepare_fixed_enemies() heraus, wenn
	# ein echtes Kampf-Gate existiert - Tresorraeume haben nie eines
	# (_table_for_type()/_budget_for_type() liefern fuer TREASURE immer
	# leer/0) und bekamen dadurch NIE eine Dimmplatte. Kombiniert mit
	# is_room_revealed()'s "auch der Nachbar eines besuchten Raums gilt
	# schon als aufgedeckt"-Regel wurde ein Tresorraum so voll sichtbar,
	# sobald man in den Nachbarraum kam - lange vor dem eigentlichen
	# Betreten. Hier direkt beim Setzen des Raumtyps nachholen (Funktion ist
	# idempotent); das Entfernen der Platte beim ersten Betreten uebernimmt
	# _update_entry_presence().
	if type == RoomData.RoomType.TREASURE:
		_build_room_state_overlay()
...
func _build_special_room_icon() -> void:
	...
	elif room_type == RoomData.RoomType.SHOP:
		icon = _build_shop_icon()
	if icon == null:
		return
...
## Einfache Beutel-Silhouette (Shop): runder Sack + kleiner Henkel-Balken
## obendrueber - gleiches flaches Primitiv-Baumuster wie Treasure/Boss.
## RoomGridGenerator erzeugt aktuell keine SHOP-Zellen, die Funktion steht
## fuer Konsistenz/kuenftige Nutzung bereit.
func _build_shop_icon() -> Node3D:
	var root := Node3D.new()
	root.name = "ShopIcon"

	var bag_mesh := CylinderMesh.new()
	bag_mesh.top_radius = special_room_icon_size * 0.4
	bag_mesh.bottom_radius = special_room_icon_size * 0.5
	bag_mesh.height = 0.05
	root.add_child(_make_flat_icon_mesh(bag_mesh, shop_room_icon_color))

	var handle_mesh := BoxMesh.new()
	handle_mesh.size = Vector3(special_room_icon_size * 0.4, 0.05, special_room_icon_size * 0.14)
	var handle: MeshInstance3D = _make_flat_icon_mesh(handle_mesh, shop_room_icon_color)
	handle.position = Vector3(0.0, 0.02, -special_room_icon_size * 0.32)
...
func _apply_theme_recursive(node: Node, theme: StageTheme) -> void:
	...
	#
	# BUGFIX "Treasure-/Boss-Icons auf der Minimap unsichtbar": set_room_type()
	# baut das Icon VOR apply_theme() - ohne diesen Ausschluss haette dieser
	# Rundgang die bewusst gesetzte Gold-/Rot-Farbe der Icon-Meshes
	# (_make_flat_icon_mesh(), material_override != null) mit demselben
	# WAND-Farbton uebermalt wie "Ceiling" oben - exakt dasselbe Bug-Muster,
	# hier nur an den neu hinzugekommenen Icon-Wurzeln nicht mitgezogen.
	if mesh != null and not mesh.name.to_lower().contains("voidshaft"):
		...
		if parent_name != "Ceiling" and parent_name != "TreasureIcon" and parent_name != "BossIcon" and parent_name != "ShopIcon":
			...
			if is_wall and theme.wall_texture != null:
				...
				# Nur der Boden wird abgedunkelt - Rueckmeldung "Schachbrett-
				# Textur des Bodens dunkler machen", gleiche Idee wie beim
				# Wand-Texture-Darken oben.
				tint = tint * StageTheme.FLOOR_TEXTURE_DARKEN
				tint.a = 1.0
