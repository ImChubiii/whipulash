...
func show_win() -> void:
	...
	# Tutorial-Abschluss: war dieses Spiel ein Tutorial-Run, jetzt den
	# Tutorial-Completed-Status setzen (entsperrt Normal/Speedrun im Menue).
	var tutorial_flow: Node = get_node_or_null("/root/TutorialFlow")
	if tutorial_flow != null and bool(tutorial_flow.get("was_active")):
		tutorial_flow.set("was_active", false)
		GameStats.complete_tutorial()
