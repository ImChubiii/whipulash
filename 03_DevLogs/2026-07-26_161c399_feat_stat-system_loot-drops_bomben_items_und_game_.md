...
Neues Fundament für Progression und Trefferfeedback. Bewusst so gebaut,
...
- Zentrale Werte für Schaden, Tempo, Angriffstempo, Glueck, Magnetradius,
...
- Haengt sich über SceneTree.node_added an die Räume, NICHT über den
...
- Eigener RNG pro Raum, abgeleitet aus Run-Seed und Rasterposition. Der
  globale RNG laeuft laut det_rng.gd auch für Shake und Schadenszahlen —
...
- Fallback-Position ist der Spieler, nicht get_room_center(): saemtliche
  Combat-Räume haben (noch) keine LootSpawnPoints, und get_room_center()
...
- Schiebbar über einen eigenen Push-Bereich: CharacterBody3D uebertraegt in
...
- Explosion mit Entfernungs-Abfall (am Rand noch 40 %), Kettenreaktion über
...
- Bluten laeuft über den bestehenden StatusEffectManager statt über eine
...
- Starthilfekabel setzt den Dash-Zustand von CombatBase direkt, statt eine
  eigene Bewegung zu bauen — laeuft damit über denselben getesteten
  Codepfad inklusive Federarm-Schutz.

Game Juice (game_juice.gd)
- Hit-Stop über Engine.time_scale. Kurz bei Primary, laenger bei Secondary,
  am laengsten bei Explosionen.
- Der Restore-Timer laeuft über die Systemzeit, nicht über delta oder
