...
func _ready() -> void:
	if cached_frames != null:
		...
		for i in range(0, 6):
			var path := "res://assets/vfx/firefx/firefx003/fire_fx_0031%d.png" % i
		...
		if loaded_count == 0:
			push_error("VFX: Fire-Bilder konnten nicht geladen werden!")
		else:
	...
	# Sehr groß, wie vom User gewünscht
	self.pixel_size = 0.12
