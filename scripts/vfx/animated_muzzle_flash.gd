...
func _ready() -> void:
	if cached_frames != null:
		...
		frames.set_animation_speed("default", 16) # Sehr schnell für Muzzle Flash
		
		var loaded_count: int = 0
		for i in range(0, 5):
			var path := "res://assets/vfx/explosion/explosion001/explosion_0011%d.png" % i
		...
		if loaded_count == 0:
			push_error("VFX: Explosion-Bilder konnten nicht geladen werden!")
		else:
