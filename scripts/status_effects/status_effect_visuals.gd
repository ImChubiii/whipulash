...
func _process(delta: float) -> void:
	...
	var strength: float = _strength_for(id) * (1.0 - PULSE_AMOUNT + PULSE_AMOUNT * (0.5 + 0.5 * sin(_pulse)))
...
## Der Farbkreis-Trick fuer "confused" (HOLOGRAM_RAINBOW aus dem Design-
## Dokument): die Grundfarbe wird ueber die Zeit im HSV-Raum gedreht.
func _color_for(id: String) -> Color:
	match id:
		...
		StatusBurn.ID:
			return StatusBurn.TINT_COLOR
		StatusAcid.ID:
			return StatusAcid.TINT_COLOR
		StatusSlow.ID:
			return StatusSlow.TINT_COLOR
		StatusConfused.ID:
			var hue: float = fmod(Time.get_ticks_msec() * 0.001 * StatusConfused.RAINBOW_SPEED, 1.0)
			return Color.from_hsv(hue, 0.75, 1.0)
		"bleed":
			return Color(0.85, 0.10, 0.12)
		"poison":
			return Color(0.45, 0.85, 0.30)
	return Color(1.0, 1.0, 1.0)


func _strength_for(id: String) -> float:
	match id:
		StatusStun.ID:
		...
		StatusBurn.ID:
			return StatusBurn.TINT_STRENGTH
		StatusAcid.ID:
			return StatusAcid.TINT_STRENGTH
		StatusSlow.ID:
			return StatusSlow.TINT_STRENGTH
		StatusConfused.ID:
			return StatusConfused.TINT_STRENGTH
	return 0.30
