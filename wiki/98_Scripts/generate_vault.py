...
liegen), sondern die echten Projektdateien selbst:
    ...
    game/scripts/items/item_catalog.gd        -> wiki/01_Game_Design/Items
    game/resources/enemies/es_*.tres          -> wiki/01_Game_Design/Enemies
    game/scenes/{enemies,scout_dummy,tank_dummy}.tscn (Balancing-Werte)
    game/resources/rooms/rd_*.tres            -> wiki/01_Game_Design/Rooms
    game/scripts/status_effects/*.gd          -> wiki/01_Game_Design/Status_Effects
    git log (repo-weit)                       -> wiki/03_DevLogs
...
Aufruf:
    python wiki/98_Scripts/generate_vault.py
...
Repo-Layout (seit dem wiki/game-Split): dieses Skript liegt unter
wiki/98_Scripts/generate_vault.py. GAME_ROOT zeigt auf das Godot-Projekt
(../../game von hier aus), WIKI_ROOT auf den Vault-Root (..). git log laeuft
bewusst mit cwd=REPO_ROOT (der echte, gemeinsame Root ueber game/ und wiki/),
damit die Commit-Historie repo-weit bleibt statt nur game/-Commits zu zeigen.
"""
...
def load_graphify_data() -> None:
    ...
    graph_path = WIKI_ROOT / "graphify-out" / "graph.json"
...
def write_templates() -> None:
    write_md(WIKI_ROOT / "99_Templates/tpl_Item.md", TPL_ITEM)
    write_md(WIKI_ROOT / "99_Templates/tpl_Enemy.md", TPL_ENEMY)
    write_md(WIKI_ROOT / "99_Templates/tpl_Room.md", TPL_ROOM)
    write_md(WIKI_ROOT / "99_Templates/tpl_StatusEffect.md", TPL_STATUS_EFFECT)
...
def parse_items() -> list[dict]:
    src = (GAME_ROOT / "scripts/items/item_catalog.gd").read_text(encoding="utf-8")
...
def write_item_notes(items: list[dict], item_cross_refs: dict[str, dict[str, list[str]]],
    ...
    out_dir = WIKI_ROOT / "01_Game_Design/Items"
...
def parse_item_cross_references(items: list[dict]) -> dict[str, dict[str, list[str]]]:
    ...
    """
    behaviours_path = GAME_ROOT / "scripts/items/item_behaviours.gd"
...
def parse_enemies() -> list[dict]:
    ...
    for display_name, scene_rel in ENEMY_SCENES.items():
        scene_text = (GAME_ROOT / scene_rel).read_text(encoding="utf-8")
        ...
        spawn_kv = {}
        if spawn_entry_rel and (GAME_ROOT / spawn_entry_rel).exists():
            spawn_kv = parse_tres_kv((GAME_ROOT / spawn_entry_rel).read_text(encoding="utf-8"))
...
def write_enemy_notes(enemies: list[dict],
                       entity_mentions: dict[tuple[str, str], list[dict]] | None = None) -> None:
    out_dir = WIKI_ROOT / "01_Game_Design/Enemies"
    for e in enemies:
...
def parse_characters() -> list[dict]:
    ...
    for char_id, tres_rel in CHARACTER_RESOURCES.items():
        tres_kv = parse_tres_kv((GAME_ROOT / tres_rel).read_text(encoding="utf-8"))
        scene_text = (GAME_ROOT / CHARACTER_SCENES[char_id]).read_text(encoding="utf-8")
        combat_text = (GAME_ROOT / CHARACTER_COMBAT_SCRIPTS[char_id]).read_text(encoding="utf-8")
...
def write_character_notes(characters: list[dict],
                           entity_mentions: dict[tuple[str, str], list[dict]] | None = None) -> None:
    out_dir = WIKI_ROOT / "01_Game_Design/Characters"
...
def find_room_scene(room_id: str) -> str:
    matches = list((GAME_ROOT / "scenes/rooms").rglob(f"room_{room_id}.tscn"))
    if matches:
        return str(matches[0].relative_to(GAME_ROOT)).replace("\\", "/")
    # room_start_01.tres liegt direkt unter scenes/rooms/, ohne Unterordner
    direct = GAME_ROOT / "scenes/rooms" / f"room_{room_id}.tscn"
    if direct.exists():
        return str(direct.relative_to(GAME_ROOT)).replace("\\", "/")
    return ""


def parse_rooms() -> list[dict]:
    rooms = []
    for tres_path in sorted((GAME_ROOT / "resources/rooms").glob("rd_*.tres")):
...
def write_room_notes(rooms: list[dict],
                      entity_mentions: dict[tuple[str, str], list[dict]] | None = None) -> None:
    out_dir = WIKI_ROOT / "01_Game_Design/Rooms"
    for r in rooms:
        body = f"""---
id: {yaml_escape(r['id'])}
...
def parse_status_effects() -> list[dict]:
    ...
    for name in STATUS_EFFECT_FILES:
        path = GAME_ROOT / f"scripts/status_effects/{name}.gd"
...
def write_status_effect_notes(effects: list[dict], status_item_links: dict[str, list[str]],
                               ...
                               entity_mentions: dict[tuple[str, str], list[dict]] | None = None) -> None:
    out_dir = WIKI_ROOT / "01_Game_Design/Status_Effects"
...
def write_architecture_notes(entity_mentions: dict[tuple[str, str], list[dict]] | None = None) -> None:
    out_dir = WIKI_ROOT / "02_Tech_Architecture"
...
{chr(10).join(blocks)}
"""
    write_md(WIKI_ROOT / "03_DevLogs/_MOC_DevLogs.md", content)
...
"""
    write_md(WIKI_ROOT / "01_Game_Design/Items/_MOC_Items.md", items_content)
...
"""
    write_md(WIKI_ROOT / "01_Game_Design/Enemies/_MOC_Enemies.md", enemies_content)
...
"""
        write_md(WIKI_ROOT / "01_Game_Design/Characters/_MOC_Characters.md", characters_content)
...
"""
    write_md(WIKI_ROOT / "01_Game_Design/Rooms/_MOC_Rooms.md", rooms_content)
...
"""
    write_md(WIKI_ROOT / "01_Game_Design/Status_Effects/_MOC_Status_Effects.md", effects_content)
...
"""
    write_md(WIKI_ROOT / "00_Dashboard/00_Master_Wiki.md", content)
...
Aufruf:
    python wiki/98_Scripts/wiki_sync.py            # Dry-Run, zeigt nur Diffs
    python wiki/98_Scripts/wiki_sync.py --apply     # schreibt tatsaechlich
"""
...
WIKI_ROOT = Path(__file__).resolve().parent.parent
GAME_ROOT = WIKI_ROOT.parent / "game"
...
def sync_items() -> int:
    ...
    catalog = GAME_ROOT / "scripts/items/item_catalog.gd"
    items_dir = WIKI_ROOT / "01_Game_Design/Items"
...
def sync_rooms() -> int:
    ...
    rooms_res_dir = GAME_ROOT / "resources/rooms"
    rooms_notes_dir = WIKI_ROOT / "01_Game_Design/Rooms"
...
def write_wiki_sync() -> None:
    (WIKI_ROOT / "98_Scripts/wiki_sync.py").write_text(WIKI_SYNC_TEMPLATE, encoding="utf-8")
