...
Laufzeit-Komponente pro Entity (Spieler wie Gegner) für alle aktiven
...
- `apply_effect(id, duration, magnitude, source, tick_interval)` — **nimmt
  ...
  Für Verlaengerungen ungeeignet: eine Pfeffermuehle mit +3s waere bei
...
- `get_effect_tick_interval(id)`, `snapshot_dots()` — für Synergie-Rechnungen
...
- **Dauer-Tint verschwand beim ersten Treffer:** `psx.gdshader` hat GENAU
  ...
  Effekt-Einfaerbung wurde beim nächsten Schlag geloescht. Fix:
...
- **Stun-Interrupt haette Gegner dauerhaft gelaehmt:** `_do_attack()` ist
  eine Coroutine über mehrere `await`-Punkte. Ein reines `return` beim
...
- [[custom_enemy_base]] — zweite Anbindungsstelle für die sechs neuen
